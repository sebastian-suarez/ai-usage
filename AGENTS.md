...
- **Project fields:** `Status` (Backlog · Planned · Implementing · In review ·
  Blocked · Done · Not planned), `Item type`, `Priority` (P0 · P1 · P2), `Branch`
  (`<type>/<slug>`). Done and Not planned are closed issues (reasons "completed"
  and "not planned").
...
- Never delete issues — set Done, Not planned, or move back to Backlog. The only
  exception is an item that never belonged to this project at all.
...
Blocked = needs the user · Done = reviewed, merged and closed · Not planned =
closed without implementing (superseded, invalid, won't do), kept for traceability.
